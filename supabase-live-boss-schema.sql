...
-- Correctif du 2026-07-08 : quand la réclamation échoue parce que le joueur a DÉJÀ réclamé (le
-- vrai cas de double-réclamation/exploit, distinct des 2 autres raisons de retour -1), une alerte
-- part directement vers le salon Discord "cheat" (même webhook que notify_cheat_discord, voir
-- supabase-anticheat-schema.sql) — déplacé depuis le client (qui l'envoyait au salon GÉNÉRAL via
-- logToDiscord) : plus fiable côté serveur, et ne peut pas être contourné par le client.
...
begin
  ...
  if exists (select 1 from public.boss_claims where boss_key = v_key and user_id = v_uid) then -- déjà réclamé : vrai cas de double-réclamation
    begin
    ...
    exception when others then null; -- ne jamais faire échouer la réclamation si Discord est indisponible
    end;
    return -1;
  end if;
