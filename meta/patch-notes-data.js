...
// inventer une.
const PATCH_NOTES = [
  { v:'V497', d:'24/07/2026 03:00', name:{fr:'Tutoriels : la pop-up amène le menu concerné à l\'écran', en:'Tutorials: the pop-up brings the relevant menu into view'}, fr:[
  ...
  { v:'V485', d:'19/07/2026 11:00', name:{fr:'Familier ramasseur en combat', en:'Loot-collecting companion in combat'}, fr:[
      ...
      {t:'new', sub:'compagnon', tx:'Your Gathering companion now follows you in combat and picks up the loot dropped on the ground by monsters! It darts to items and grabs them alongside your character — especially handy in XP mode, where your character ignores ground loot. Everything lands in the same bag as usual, with a small paw 🐾 in the loot ticker to show the companion collected it. Gathering-type companions do the work: their "Collection Speed" and "Radius" stats drive the companion\'s movement speed and pickup range (the one deployed on the Gathering field takes priority).'},
    ] },
  ...
  { v:'V477', d:'18/07/2026 16:00', name:{fr:'Compagnon passe en prod : silver lié au jeu, vraies valeurs, farm ×5', en:'Companions go live: silver linked to the game, real values, ×5 farming'}, fr:[
      ...
      {t:'new', sub:'compagnon', tx:'Tes compagnons farment 5× plus : chaque butin (objets ET silver), en jeu comme hors-ligne, rapporte 5 fois plus qu\'avant.'},
      {t:'change', sub:'compagnon', tx:'Au passage en prod, la progression Compagnon repart d\'une base propre : familiers, inventaire et slots d\'incubation sont remis à zéro (les stocks accumulés pendant les tests fausseraient la nouvelle économie). Ton silver (celui du jeu) et tes succès déjà obtenus sont CONSERVÉS.'},
    ], en:[
      {t:'change', sub:'compagnon', tx:'The Companions module is no longer "in testing": its silver is now THE SAME as the main game\'s. Buying an egg or an incubation slot spends your game silver; your companions\' loot and sales credit it. Everything is tracked like any other source (category "companion"). No more separate purse: what your companions earn funds everything else, and vice versa.'},
      {t:'change', sub:'compagnon', tx:'Prices and times are now real values (no more divided test values). Hatching an egg takes 6h. There are now 5 incubation slots total: 2 free from the start, then 3 to unlock in order for 1M, 10M and 100M silver.'},
      {t:'new', sub:'compagnon', tx:'Your companions farm 5× more: every bit of loot (items AND silver), live or offline, yields five times what it did before.'},
      {t:'change', sub:'compagnon', tx:'With the switch to production, Companion progress starts from a clean slate: pets, inventory and incubation slots are reset (test-era stockpiles would skew the new economy). Your silver (the game\'s) and any achievements you already earned are KEPT.'},
    ] },
  { v:'V476', d:'18/07/2026 09:00', name:{fr:'Accessibilité clavier & mouvement réduit, et un jeu plus léger', en:'Keyboard accessibility & reduced motion, and a lighter game'}, fr:[
      {t:'new', sub:'accessibilite', tx:'Les fenêtres (patch notes, wiki, marché, compte, admin…) se ferment maintenant avec la touche Échap, et le focus clavier reste piégé à l\'intérieur tant qu\'elles sont ouvertes — Tab ne s\'échappe plus vers le décor derrière. À la fermeture, le focus revient sur le bouton qui avait ouvert la fenêtre. Deux exceptions volontaires : l\'écran de connexion sans compte et l\'écran « jeu en pause » (autre session active) ne se ferment PAS avec Échap, c\'est leur rôle.'},
      {t:'new', sub:'accessibilite', tx:'Si ton système est réglé sur « réduire les animations » (option Windows, macOS, Android ou iOS), le jeu respecte désormais ce choix : pulsations, halos, rebonds et surtout la grande roue de récompense de boss (5 tours sur elle-même) sont neutralisés. Aucune information n\'est perdue — la roue affiche directement la récompense obtenue. Le jeu continue de tourner normalement, seul le mouvement décoratif est retiré.'},
      {t:'change', sub:'chargement', tx:'Le jeu vérifie l\'arrivée d\'une nouvelle version toutes les 60 secondes. Il retéléchargeait jusqu\'ici tout l\'historique des patch notes à chaque vérification (~184 Ko, sans cache) — environ 11 Mo par heure juste pour lire un numéro de version. Il ne lit plus qu\'un fichier de 14 octets. En prime, le comptage des joueurs en ligne / zones / Velia passe d\'une requête toutes les 20 s à une seule regroupée : moins de données pour toi, moins de charge serveur.'},
    ], en:[
      {t:'new', sub:'accessibilite', tx:'Panels (patch notes, wiki, market, account, admin…) now close with the Escape key, and keyboard focus stays trapped inside them while open — Tab no longer escapes to the scene behind. On close, focus returns to the button that opened the panel. Two deliberate exceptions: the sign-in screen without an account and the "game paused" screen (another active session) do NOT close with Escape, by design.'},
      {t:'new', sub:'accessibilite', tx:'If your system is set to "reduce motion" (a Windows, macOS, Android or iOS option), the game now honors it: pulses, glows, bounces and especially the big boss reward wheel (5 full spins) are stilled. No information is lost — the wheel shows the reward directly. The game itself keeps running normally; only decorative motion is removed.'},
      {t:'change', sub:'chargement', tx:'The game checks for a new version every 60 seconds. Until now it re-downloaded the entire patch-note history on every check (~184 KB, uncached) — roughly 11 MB per hour just to read a version number. It now reads a single 14-byte file. As a bonus, the online-players / zones / Velia counts went from one request every 20s to a single grouped one: less data for you, less server load.'},
    ] },
  ...
  { v:'V474', d:'17/07/2026 10:30', name:{fr:'Correctif : plus de niveau fantôme ni de faux résumé hors-ligne à la création de compte', en:'Fix: no more phantom level or fake offline summary on account creation'}, fr:[
      ...
      {t:'fix', sub:'connexion', tx:'Plus rien du jeu ne tourne tant qu\'aucun compte n\'est chargé : le temps passé sur l\'écran de connexion n\'est plus compté comme du temps de JEU, et aucune sauvegarde fantôme n\'est écrite. Le cadeau de bienvenue de 80 silver est désormais accordé à la création du personnage — il était jusqu\'ici crédité à tout visiteur 1,2 s après l\'ouverture de la page, et pouvait même être re-crédité à chaque connexion selon la vitesse de chargement.'},
    ], en:[
      ...
      {t:'fix', sub:'connexion', tx:'Nothing game-related runs until an account is loaded: time spent on the sign-in screen no longer counts as PLAYTIME, and no phantom save is written. The 80 silver welcome gift is now granted on character creation — until now it was credited to every visitor 1.2s after page load, and could even be re-credited on every login depending on load speed.'},
    ] },
