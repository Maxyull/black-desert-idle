  ...
  document.getElementById('sec-nav').innerHTML=SECTIONS.map((s,i)=>{
    ...
    return`<div class="sec-row ${i===activeSecIdx?'active':''}" onclick="activeSecIdx=${i};renderSecNav();renderSecDetail()">
      ...
      <div style="flex:1;min-width:0">
        <div style="font-family:'Cinzel',serif;font-size:10px;color:${i===activeSecIdx?'var(--cream)':'var(--cream2)'}">${secName(s)}</div>
        ${tp?`<div style="font-size:9px;color:var(--cream3);margin-top:1px">${tp.cat.name}</div>`:`<div style="font-size:9px;color:var(--cream3)">${i18next.t('companions:companions.sections.no_pet')}</div>`}
      </div>
...
function renderSecDetail(){
  ...
  const terrainHtml=tp?`
    <div class="terrain-slot occ" style="--pcard-color:${rc(tp.rar)}">
      ...
      <div class="pcard-name">
        ...
        <div style="font-size:9px;color:${rc(tp.rar)};margin-top:2px">${rn(tp.rar)} · ${typeLabel(tp.cat.typ)}</div>
      </div>
      <div class="pcard-body">
        <div style="display:flex;align-items:center;gap:7px">
          <span style="font-size:10px;color:var(--cream2)">${i18next.t('companions:companions.hatch.gs_of_max', {pct:gsPct(tp), rarity:rn(tp.rar)})}</span>
        </div>
        ${renderTierBlock(tp)}
        ${renderStatBars(tp)}
      ...
      <div class="pcard-actions">
        ${typeof companionModelUrlFor==='function'&&companionModelUrlFor(tp)?`<button class="btn btn-ghost" style="font-size:9px;padding:4px 9px;flex:1" onclick="open3dPreviewModal(PETS.find(p=>p.id===${tp.id}))">${i18next.t('companions:companions.collection.view_3d_btn')}</button>`:''}
        <button class="btn btn-red" style="font-size:9px;padding:4px 9px;flex:1" onclick="PETS.find(p=>p.id===${tp.id}).terrain=false;renderAll()">${i18next.t('companions:companions.collection.remove_btn')}</button>
      </div>
    </div>`:`
    <div class="terrain-slot">
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;gap:6px;flex:1;color:var(--cream3)">
        ...
        <span style="font-size:11px">${i18next.t('companions:companions.sections.no_pet_on_field')}</span>
        <button class="btn btn-ghost" style="margin-top:3px" onclick="ST(2)">${i18next.t('companions:companions.sections.choose_from_collection')}</button>
      </div>
    </div>`;
  ...
  const resHtml=reserves.length?`
    <div style="background:var(--s2);border:1px solid var(--border);border-radius:9px;overflow:hidden;display:flex;flex-direction:column;min-height:0">
      <div style="padding:4px 8px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:6px;flex-wrap:wrap;flex-shrink:0">
        <span style="font-family:'Cinzel',serif;font-size:8px;letter-spacing:.08em;color:var(--cream2)">${i18next.t('companions:companions.sections.reserve_title', {count:reserves.length})}</span>
            ...
            <div style="padding:3px 5px;cursor:pointer" onclick="toggleResPetExpand(${p.id})">
              ...
              <div style="display:flex;align-items:center;gap:3px;margin-top:3px;flex-wrap:wrap">
                ...
                <div style="margin-left:auto;display:flex;gap:2px">
                  ...
                  <button style="font-size:6px;padding:1px 3px;border-radius:3px;border:1px solid var(--gold-dim);background:transparent;color:var(--gold);cursor:pointer;font-family:'Cinzel',serif;flex-shrink:0" onclick="event.stopPropagation();deployPet(${p.id})">${i18next.t('companions:companions.hatch.deploy_btn')}</button>
                </div>
              </div>
            </div>
  ...
  c.innerHTML=`
    <div style="display:flex;align-items:center;gap:10px">
      ...
      <div><div style="font-family:'Cinzel',serif;font-size:17px;color:var(--cream)">${secName(s)}</div><div style="font-size:11px;color:var(--green2)">${s.bonus||''}</div></div>
    </div>
    <div style="display:flex;gap:14px;align-items:flex-start">
      <div style="width:300px;flex-shrink:0;background:var(--s2);border:1px solid var(--border);border-radius:10px;overflow:hidden">
        <div style="padding:7px 12px;border-bottom:1px solid var(--border);font-family:'Cinzel',serif;font-size:10px;letter-spacing:.08em;color:var(--cream2)">${i18next.t('companions:companions.sections.on_field_title')}</div>
...
function deployPet(id){
  ...
  p.terrain=true;renderAll();toast('🌿',i18next.t('companions:companions.hatch.deployed_toast', {name:p.cat.name}));
}
