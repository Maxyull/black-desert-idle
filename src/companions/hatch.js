...
function renderHatch(){
  ...
  document.getElementById('incub-slots').innerHTML=incubSlots.map((sl,i)=>{
    ...
    const eggIco = eggType.ico;
    // "Gratuit" reflète l'ŒUF en incubation (basique, coût 0), pas le slot (2026-07-19, bug rapporté :
    // le badge restait affiché après achat d'un œuf payant dans un slot gratuit). sl.free = slot sans
    // coût de déblocage (les 2 premiers), un concept distinct utilisé ailleurs (slot vide / modale).
    const eggFree = eggType.cost === 0;
    if(sl.ready)return`<div class="isl ready"><div style="position:relative"><span style="font-size:28px">${eggIco}</span><div style="position:absolute;inset:-6px;border-radius:50%;background:radial-gradient(circle,rgba(111,220,111,.4),transparent);animation:eglaur 1s ease-in-out infinite"></div></div>${eggFree?'<span style="font-size:8px;color:var(--green2);background:rgba(111,220,111,.1);border:1px solid rgba(111,220,111,.3);border-radius:3px;padding:1px 4px">'+i18next.t('companions:companions.hatch.free_badge')+'</span>':''}<div class="itimer done">${i18next.t('companions:companions.hatch.ready_badge')}</div><button style="font-family:Cinzel,serif;font-size:9px;padding:4px 12px;border-radius:4px;border:1px solid var(--gold);background:linear-gradient(135deg,var(--gold-dim),var(--gold));color:var(--bg);cursor:pointer;margin-top:2px" onclick="doHatch(${i})">${i18next.t('companions:companions.hatch.hatch_btn')}</button></div>`;
    const pct=Math.round((1-sl.tl/sl.tot)*100);
    return`<div class="isl">${eggFree?'<span style="font-size:8px;color:var(--green2);background:rgba(111,220,111,.1);border:1px solid rgba(111,220,111,.3);border-radius:3px;padding:1px 4px">'+i18next.t('companions:companions.hatch.free_badge')+'</span>':''}<span style="font-size:28px">${eggIco}</span><div class="itimer">${fmtT(sl.tl)}</div><div class="iprog"><div class="iprog-fill" style="width:${pct}%"></div></div></div>`;
  }).join('');
  // Odds
